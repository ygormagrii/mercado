...
<div class="col-md-12">
    <div class="card">
        <div class="header">
            <h4 class="title">Instruções de Trabalho</h4>
        </div>
        <div class="content table-responsive table-full-width">
            <table class="table table-hover table-striped">
                    <thead>
                        ...
                        <th class="no-mobile">Data de geração</th>
                    </tr>
                    </thead>
                    <tbody>
                        ...
                        ?>
                        <tr>
                            <td class="yes-mobile" style="display: none;"><?= $produto["id"]?></td>
                            <td class="no-mobile"><?= anchor("idt/{$produto['id']}", $produto["nome"]); ?></td>
                            <td class="no-mobile"><?= $produto["data"]?></td>
                            <td><?= anchor("idt/{$produto['id']}", "Imprimir / Visualizar"); ?></td>
                            <td><?= anchor("idt/excluir/{$produto['id']}", "Excluir"); ?></td>
                        </tr>
                        <?php endforeach ?>
                    </tbody>
                </table>
        ...
        <div class="card">
        <div class="header">
                <h4 class="title">IDT's Importadas</h4>
            </div>
            <div class="content table-responsive table-full-width">
                <table class="table table-hover table-striped">
                        <thead>
                        ...
                        <tbody>
                            <?php foreach($idt_importados as $importados) : ?>
                            <tr>
                                <td><?=$importados["id"]?></td>
                                <?php $caminho_arquivo = substr($importados["caminho_arquivo"], -26); ?>
                                <td class="no-mobile"><a href="http://lotopro.com.br/sistema/<?=$caminho_arquivo?>" target="_blank    "><?=$importados["nome"]?></a></td>
                                <td class="no-mobile"><?=$importados["tipo"]?></td>
                                <td class="no-mobile"><?= character_limiter($importados["descricao"], 10) ?></td>
                                <td class="no-mobile"><?=$importados["data"]?></td>
                                <td><a href="http://lotopro.com.br/sistema/<?=$caminho_arquivo?>" target="_blank">Imprimir / Visualizar</a></td>
                                <td><?= anchor("idt/exclui_importados/{$importados['id']}", "Excluir"); ?></td>
                            </tr>
                            <?php endforeach ?>
                        </tbody>
                    </table>
        ...
        <?= anchor('idt/importar','Importar Nova Instrução de Trabalho', array("class" => "btn btn-primary"))?>

        <?= anchor('idt/formulario','Adicionar Nova Instrução de Trabalho', array("class" => "btn btn-success"))?>
